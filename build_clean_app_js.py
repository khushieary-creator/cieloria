...
for row in reader:
    ...
    products.append({
        ...
        "rating": round(4.8 + (len(products) % 3) * 0.08, 2),
        "reviewCount": 120 + (len(products) * 7) % 400,
        ...
        "description": title + " - 18K Gold Plated PVD Stainless Steel anti-tarnish jewelry.",
...
js_content = """// Cieloria Application - Clean 78 Shopify Products Database

// 1. Complete Real Cieloria Shopify Product Catalog (DECLARATION FIRST)
...
// 2. Hero Slides & Categorization (DECLARED AFTER PRODUCTS)
const HERO_SLIDES = [
  {
    id: 'slide-1',
...
const CATEGORY_CARDS = [
  ...
  { id: 'necklaces', name: 'Necklaces', image: PRODUCTS[40].image, count: '28 Designs' },
  { id: 'earrings', name: 'Earrings', image: PRODUCTS[30].image, count: '18 Designs' },
...
const REELS_DATA = [
  {
    ...
    productTagged: PRODUCTS[40].name,
  ...
  return `
    ...
    <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-8">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div class="relative rounded-3xl overflow-hidden shadow-xl group h-96 bg-[#1A1A1A]">
          <img src="${PRODUCTS[40].image}" class="w-full h-full object-cover opacity-70 group-hover:scale-105 transition-transform duration-700" />
          <div class="absolute inset-0 p-8 flex flex-col justify-end text-left bg-gradient-to-t from-black/80 via-transparent to-transparent">
...
try { renderApp(); } catch(err) { console.error('Immediate render error:', err); }
"""

with open("/Users/khushi/.gemini/antigravity/scratch/cieloria/app.js", "w") as f:
    f.write(js_content)

print("Successfully regenerated clean app.js with PRODUCTS declared FIRST!")
